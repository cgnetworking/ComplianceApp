...
def assessment_script_contents(run: ZeroTrustAssessmentRun, output_root: Path) -> str:
    ...
    script_lines = [
        ...
        "  # Work around module 2.2.0 passing false switch values into Graph/Azure auth cmdlets,",
        "  # which breaks the app-certificate parameter sets used by the worker.",
        ...
        "    if ($PSBoundParameters.ContainsKey('Scopes')) { $connectMgGraphParams.Scopes = $Scopes }",
        "    if ($PSBoundParameters.ContainsKey('TenantId')) { $connectMgGraphParams.TenantId = $TenantId }",
        "    if ($PSBoundParameters.ContainsKey('ClientId')) { $connectMgGraphParams.ClientId = $ClientId }",
        "    if ($PSBoundParameters.ContainsKey('Certificate')) { $connectMgGraphParams.Certificate = $Certificate }",
        "    if ($PSBoundParameters.ContainsKey('Environment')) { $connectMgGraphParams.Environment = $Environment }",
        "    if ($PSBoundParameters.ContainsKey('ContextScope')) { $connectMgGraphParams.ContextScope = $ContextScope }",
        "    if ($PSBoundParameters.ContainsKey('NoWelcome') -and $NoWelcome.IsPresent) { $connectMgGraphParams.NoWelcome = $true }",
        "    if ($PSBoundParameters.ContainsKey('UseDeviceCode') -and $UseDeviceCode.IsPresent) { $connectMgGraphParams.UseDeviceCode = $true }",
        "    Microsoft.Graph.Authentication\\Connect-MgGraph @connectMgGraphParams",
        "  }",
        ...
        "    if ($PSBoundParameters.ContainsKey('Environment')) { $connectAzAccountParams.Environment = $Environment }",
        "    if ($PSBoundParameters.ContainsKey('Tenant')) { $connectAzAccountParams.Tenant = $Tenant }",
        "    if ($PSBoundParameters.ContainsKey('ApplicationId')) { $connectAzAccountParams.ApplicationId = $ApplicationId }",
        "    if ($PSBoundParameters.ContainsKey('CertificateThumbprint')) { $connectAzAccountParams.CertificateThumbprint = $CertificateThumbprint }",
        "    if ($PSBoundParameters.ContainsKey('UseDeviceAuthentication') -and $UseDeviceAuthentication.IsPresent) { $connectAzAccountParams.UseDeviceAuthentication = $true }",
        "    if ($connectAzAccountParams.ContainsKey('ApplicationId') -and $connectAzAccountParams.ContainsKey('CertificateThumbprint')) {",
        "      $connectAzAccountParams.ServicePrincipal = $true",
        "    }",
        "    Az.Accounts\\Connect-AzAccount @connectAzAccountParams",
        "  }",
        ...
        "  } catch {",
        "    throw ('Unable to load the generated assessment PFX using the worker credential at ' + $pfxPasswordPath + '. The web app and assessment worker may be using different assessment-pfx-password secrets. If the credential changed, regenerate the certificate in the portal and rerun the assessment. Inner error: ' + $_.Exception.Message)",
        "  }",
        "  Remove-Variable pfxPassword -ErrorAction SilentlyContinue",
        "  $certificateAddedToStore = $false",
        "  $certificateStore = [System.Security.Cryptography.X509Certificates.X509Store]::new('My', [System.Security.Cryptography.X509Certificates.StoreLocation]::CurrentUser)",
        "  $certificateStore.Open([System.Security.Cryptography.X509Certificates.OpenFlags]::ReadWrite)",
        "  try {",
        "    $storedCertificate = $certificateStore.Certificates.Find([System.Security.Cryptography.X509Certificates.X509FindType]::FindByThumbprint, $certificate.Thumbprint, $false) | Select-Object -First 1",
        "    if (-not $storedCertificate) {",
        "      $certificateStore.Add($certificate)",
        "      $certificateAddedToStore = $true",
        "    }",
        "  } finally {",
        "    $certificateStore.Close()",
        "  }",
        "  try {",
        f"    Connect-ZtAssessment -ClientId {powershell_literal(run.profile.client_id)} -TenantId {powershell_literal(run.profile.tenant_id)} -Certificate $certificate -Service 'Graph','Azure' -Force",
        f"    Invoke-ZtAssessment -Path {powershell_literal(str(output_root))} -ExportLog",
        "  } finally {",
        "    if ($certificateAddedToStore) {",
        "      $cleanupStore = [System.Security.Cryptography.X509Certificates.X509Store]::new('My', [System.Security.Cryptography.X509Certificates.StoreLocation]::CurrentUser)",
        "      $cleanupStore.Open([System.Security.Cryptography.X509Certificates.OpenFlags]::ReadWrite)",
        "      try {",
        "        $cleanupCandidates = $cleanupStore.Certificates.Find([System.Security.Cryptography.X509Certificates.X509FindType]::FindByThumbprint, $certificate.Thumbprint, $false)",
        "        foreach ($cleanupCertificate in $cleanupCandidates) {",
